...
class DescriptionPopup(QDialog):
    ...
    def save_description(self):
        self.description = self.text_edit.toPlainText()
...
class RegisterViewer(QMainWindow):
    ...
    def show_description_popup(self, item, global_pos):
        ...
        if description:
            ...
            if popup.exec_() == QDialog.Accepted:  # 如果用户点击了保存按钮
                new_description = popup.description
                item.setText(4, new_description)  # 更新树形控件中的描述

                # 更新 DataFrame
                register_name = item.text(0)
                field_name = item.text(0) if item.parent() else None
                if field_name:
                    self.df.loc[(self.df['NAME'] == register_name) & (self.df['Description'] == field_name), 'Description'] = new_description
                else:
                    self.df.loc[self.df['NAME'] == register_name, 'Description'] = new_description
    ...
    def save_to_pkl(self):
        if self.df is not None:
            pkl_file_name = self.file_path_value.text().rsplit('.', 1)[0] + '.pkl'
            self.df.to_pickle(pkl_file_name)
            print(f"Saved changes to {pkl_file_name}")
