...
class I2CConfigPage(QWidget):
    uart_output_signal = pyqtSignal(str)  # 用于UART的输出信号
    def __init__(self, devices, global_usb_i2c, global_device_address):
        ...
        self.uart_output_signal.connect(self.append_output)
        self.init_ui()
    ...
    def append_output(self, text):
        ...
        self.result_output.moveCursor(QTextCursor.End)
        #self.result_output.append(text)  # 使用 QTextEdit 的 append 方法
    ...
    def read_i2c_disp(self,register_address):
        ...
        try:
            data = self.usb_i2c.read(register_address)
            self.uart_output_signal.emit(f'读取 0x{register_address:02X}: 0x{data:02X}\n')
        except ValueError:
            QMessageBox.critical(self, '错误', '无效的寄存器地址')
    ...
    def write_i2c_disp(self,register_address,data):
        ...
        try:
            self.usb_i2c.write(register_address, data)
            self.uart_output_signal.emit(f'写入 0x{register_address:02X}: 0x{data:02X}\n')
        except ValueError:
            QMessageBox.critical(self, '错误', '无效的寄存器地址或数据')
    ...
    def write_uart(self, hex_string):
        try:
            ...
            # 发射信号传递要显示的字符串，而不是直接操作 QTextEdit
            self.uart_output_signal.emit(f"UART 写入: {hex_string} UART 返回: {rd_hex}\n")
            return rd_hex
        except ValueError:
    ...
    def read_uart(self, timeout=1, num_bytes=1024):
        try:
            ...
            if data is None:
                self.uart_output_signal.emit(f'警告 :未接收到数据')
            else:
                self.uart_output_signal.emit(f'接收到的十六进制数据: {data}\n')
    ...
    def edit_with_vi(self):
        utils.edit_with_vi(self)

    def update_temperature(self):
        utils.update_temperature(self)

    def set_progress_bar_style(self, temperature):
        utils.set_progress_bar_style(self, temperature)